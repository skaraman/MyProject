...
public static class BounceAdjustments {
  public static Dictionary<string, List<BounceFrame>> EsperHair { get; } = new Dictionary<string, List<BounceFrame>> {
    ...
    ["Blast"] = new List<BounceFrame> { new(-0.1f, -0.2f, .02f), new(-.13f, -.3f, .18f), new(-.66f, -.73f, .15f), new(1.21f, -1.48f, .23f), new(1.71f, -1.43f, .16f), new(1.7f, -1.31f, .26f) },
    ["BlastToKickLeft"] = new List<BounceFrame> { new(.3f, -.52f) },
    ["BlastToKickRight"] = new List<BounceFrame> { new(.81f, -.37f) },
    ["BlastToPunchLeft"] = new List<BounceFrame> { new(.84f, -.45f) },
    ["BlastToPunchRight"] = new List<BounceFrame> { new(.99f, -.36f) },
    ["BlastToStance"] = new List<BounceFrame> { new(-.14f, -.56f) },
    ["DanceToBlast"] = new List<BounceFrame> { new(-.04f, -.09f) },
    ["PunchLeftToBlast"] = new List<BounceFrame> { new(-.02f, -.26f) },
    ["PunchRightToBlast"] = new List<BounceFrame> { new(.04f, -.21f) },
    ["KickLeftToBlast"] = new List<BounceFrame> { new(-.15f, -.2f) },
    ["KickRightToBlast"] = new List<BounceFrame> { new(-.07f, -.21f) },
    ["RunToBlast"] = new List<BounceFrame> { new(-.03f, -.021f) },
    ["SprintToBlast"] = new List<BounceFrame> { new(-.04f, -.28f) },
    ["StanceToBlast"] = new List<BounceFrame> { new(-.06f, -.28f) },
    ["WalkToBlast"] = new List<BounceFrame> { new(-.02f, -.23f) },
