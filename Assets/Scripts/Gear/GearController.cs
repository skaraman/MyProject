...
public class GearController : MonoBehaviour {
  ...
  public void EquipGear() {
    ...
    foreach (KeyValuePair<string, GearItem> equip in equippedItems[activeForm]) {
      if (equip.Value == null && equip.Key != "Head") {
        continue;
      }
      var gearId = "";
      if (equip.Value == null && equip.Key == "Head") {
        gearId = activeForm + "_no_Head";
      }
      else {
        gearId = $"{equip.Value.gearId}_{equip.Key}";
      }
      if (!EsperanzaGearParts.ContainsKey(gearId)) {
        Debug.LogError($"No parts found for equipped gearId: {gearId}");
      }
      var parts = EsperanzaGearParts.gearParts[gearId];
      if (parts == null) {
        Debug.LogError($"Null parts list returned for gearId: {gearId}");
      }
      foreach (GameObject go in GearObjects) {
        foreach (string part in parts) {
          if (go != null && go.name.Equals(part)) {
            var spriteWithNormals = go.GetComponent<SpriteWithNormals>();
            var shaderAnimator = go.GetComponent<AllIn1ShaderAnimator>();
            if (spriteWithNormals != null) {
              spriteWithNormals.labelPrefix = equip.Value.gearId;
            }
            else {
              Debug.LogWarning($"GameObject {go.name} does not have a SpriteWithNormals component attached.");
            }
            if (shaderAnimator != null) {
              var ncolor = ShaderColors.myColors[equip.Value.gearColor];
              shaderAnimator.SetKeyword("GLOW_ON", true);
              shaderAnimator.AddFloatSequence("_Glow", 4f, 4f, 1f, 0f, AllIn1ShaderAnimator.LoopType.None);
              shaderAnimator.AddColorSequence("_GlowColor", ncolor, ncolor, 1f, 0f, AllIn1ShaderAnimator.LoopType.None);
            }

            else {
              Debug.LogWarning($"GameObject {go.name} does not have a AllIn1ShaderAnimator component attached.");
            }
          }
        }
      }
      foreach (GameObject bounceParent in combinedBounces) {
        ...
        foreach (Transform child in bounceParent.transform) {
          if (child != null && child.gameObject.name.Equals(gearId)) {
            ...
            var shaderAnimator = child.gameObject.GetComponent<AllIn1ShaderAnimator>();
            if (shaderAnimator != null && equip.Value != null || gearId == activeForm + "_no_Head") {
              ...
              shaderAnimator.SetKeyword("GLOW_ON", true);
              shaderAnimator.AddFloatSequence("_Glow", 6f, 6f, 1f, 0f, AllIn1ShaderAnimator.LoopType.None);
              shaderAnimator.AddColorSequence("_GlowColor", ncolor, ncolor, 1f, 0f, AllIn1ShaderAnimator.LoopType.None);
