...
half4 CombinedShapeLightShared(in SurfaceData2D surfaceData, in InputData2D inputData)
{
  ...
  // color.r += color.r < 0.5 ? (0.5 - color.r) : 0;
  // color.g += color.g < 0.5 ? (0.5 - color.g) : 0;
  // color.b += color.b < 0.5 ? (0.5 - color.b) : 0;
  ...
  half4 finalOutput = _HDREmulationScale * (color * finalModulate + finalAdditve);
#endif
